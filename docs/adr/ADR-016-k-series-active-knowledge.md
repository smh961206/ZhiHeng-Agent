...
- Runtime Knowledge loading accepts only `Kx.y.z` snapshots. Pre-K release files are no longer retained and cannot create or resume current tasks.
...
K1.0.0 lives under `knowledge/versions/auto/K1.0.0/<snapshot-id>/`. Activation is explicit and rollback cannot overwrite a snapshot. Publication-time wording and source labels inside K1.0.0 remain immutable even when their pre-K source directories have been removed; they do not create runtime filesystem dependencies. A future K release must pass lint and applicable release gates before the pointer changes. Pre-K source history is available only through Git history.
