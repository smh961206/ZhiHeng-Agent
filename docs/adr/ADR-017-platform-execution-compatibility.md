...
Maintain two active release lines: platform V-Series and Knowledge K-Series. Model engineering work ships within the platform and has no separate runtime release identity or retained release archive. Existing model configuration, modelState and contextVersion pins retain their precise compatibility responsibilities.
...
Legacy saved tasks keep their original plan, request messages, result metadata and event format, but continuation requires a retained compatible K-Series snapshot. New tasks use the current scope format. No path upgrades saved records in place, and a checkpoint cannot move between formats merely by renaming its plan field. Pre-K Knowledge artifacts are unavailable and cannot resume.
