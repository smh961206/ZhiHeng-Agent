...
Introduce explicit capability profiles while preserving old env behavior.
...
- ModelProfile schema.
- Legacy analysis/vision profile generation from existing LLM_* env.
- Capabilities and nullable pricing metadata.
...
ModelProfile is configuration/domain metadata; no research schema.

All persistent changes are additive-first. Unknown historical values remain unknown.

## 14. API

Internal catalog/profile API only.
...
- **V4.8.1-AC01:** Old env creates working Legacy Profile.
- **V4.8.1-AC02:** API keys are not serialized in Catalog.
