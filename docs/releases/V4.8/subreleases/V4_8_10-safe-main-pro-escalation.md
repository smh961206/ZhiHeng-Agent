...
Enable explicit reasoning/model escalation only for model-capability failure at safe checkpoints.
...
Before V4.8.10, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- shouldEscalate taxonomy.
- Main low→high→Pro high→Pro max.
- Normalized cross-provider context rebuild.
...
- Repeated invalid tool args.
- Repeated structured output failure.
- Data missing no escalation.
- Pending tool boundary.

Also run all existing tests affected by the inspected modules.

## 21. Acceptance Cases

- **V4.8.10-AC01:** Escalation only occurs after pending tools complete and checkpoint/context rebuild.
