# V4.9.7 — Vision fallback policy

Release: `V4.9`
Implementation Status: CURRENT implementation and offline validation. See [report](../V4_9_7-completion-report.md). Real candidate quality and production promotion remain unaccepted.

## 1. Why

Add same-capability fallback for provider/capability/format failures.
...
Before V4.9.7, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- Fallback rules
- Unreadable-input stop rule

## 7. Out of Scope

- Endless multi-model retries
...
- Vision fallback policy regression tests
...
- **V4.9.7-AC01:** Add same-capability fallback for provider/capability/format failures. without weakening Vision unverified-evidence rules.
