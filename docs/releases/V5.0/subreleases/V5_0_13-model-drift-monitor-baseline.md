# V5.0.13 — Model drift monitor baseline
...
Detect material post-promotion quality/format/tool drift using periodic benchmark.

## 2. Preconditions

- V5.0 previous accepted subrelease(s) completed.
...
Before V5.0.13, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- Drift metrics
- Champion invalidation alert

## 7. Out of Scope

- Automatic unreviewed replacement
...
Experiment/champion group is pinned per job; resume must not randomly reassign model group.
...
Benchmark fixtures are frozen/versioned; production experiment analysis must respect job creation time and model-policy version.

No future-information contamination is allowed.

## 18. Provenance

Persist benchmark version, candidate profiles, grader versions, task class and champion decision rationale.
...
MODEL_AB_ENABLED=false by default; challenger/champion production promotion requires explicit policy version.

High-risk behavior should be observable in disabled/dry-run/dual-run mode before becoming canonical where practical.

## 20. Tests

- Model drift monitor baseline unit/integration tests
...
- **V5.0.13-AC01:** Detect material post-promotion quality/format/tool drift using periodic benchmark. with versioned, reproducible evidence.

Acceptance is behavioral, not “code exists”.

## 22. Benchmark Gate

All champion decisions are derived from versioned frozen ZhiHeng benchmark runs with explicit quality thresholds and sample counts.
...
benchmark harness → baseline → challenger offline → statistical comparison → task champion → optional A/B → promotion.
