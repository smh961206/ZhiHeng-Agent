# V5.1.3 — Per-call cost calculation
...
Compute estimated cost only when pricing/usage are sufficient.
...
Before V5.1.3, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
Additive optional usage/cost/transport fields in existing ModelCalls; absent historical fields retain legacy shape and unknown semantics. See [release schema](../schema.md) and [migration](../migration.md).
...
- Per-call cost calculation tests
...
- **V5.1.3-AC01:** Compute estimated cost only when pricing/usage are sufficient. while quality remains the higher-priority gate.
