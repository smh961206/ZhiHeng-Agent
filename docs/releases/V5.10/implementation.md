...
- 组合、持仓、证券、决策、风险约束和归因使用稳定标识与规范化关系；关系查询先由主库和可重建读模型承担，不默认引入图数据库或数据仓库。
- 多币种金额、权重、收益、风险预算和优化输入使用 Decimal 与明确汇率来源/时间；优化器、求解器、目标函数和约束版本必须进入结果清单。
- 组合聚合与分析投影携带源版本、水位和陈旧标识，可从规范持仓与决策记录重建；缓存或投影缺失只能影响性能，不能丢失规范状态。
- 高频分析或历史回测只有在主库隔离、成本或性能指标证明需要时，才评估独立 OLAP/仓库，并通过 ADR 定义复制延迟和一致性边界。
- 跨证券和跨组合计算必须保留逐层依赖血缘，使风险、归因和资本配置结果能够回放。

跨版本背景见 [V5.3 至 V6.0 基础设施与数据架构演进方案](../../architecture/infrastructure-data-evolution-plan-v5.3-v6.0.md)。本节及本版本子规格是 V5.10 的执行依据。
