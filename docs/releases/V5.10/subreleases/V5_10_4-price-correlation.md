...
Add explicit-window return correlation as one risk lens.
...
Before V5.10.4, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- **V5.10.4-AC01:** Add explicit-window return correlation as one risk lens. with advisory-only behavior and no automatic holdings mutation.
