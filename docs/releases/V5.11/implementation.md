...
- 研究快照清单除领域对象外，还固定数据库模式版本、索引清单、对象哈希、解析器/规则、嵌入模型、配置、代码和运行环境版本。
- 回放必须校验数据库记录、对象内容哈希、索引版本和依赖血缘；缺失或不兼容时明确失败，不能用当前数据填补历史空洞。
- 机构记忆、搜索索引、缓存和分析投影均视为可重建派生物；重建过程必须可观察，并验证不会越过原研究截止时间。
- 只有回放、分析或在线负载实测证明主库需要隔离时，才评估只读副本或独立分析存储，并记录延迟、水位、回退和灾备边界。
- 学习保持离线、受治理和可回滚；任何新策略或模型必须经验证后发布，禁止基于线上结果自动改写规范研究状态。

跨版本背景见 [V5.3 至 V6.0 基础设施与数据架构演进方案](../../architecture/infrastructure-data-evolution-plan-v5.3-v6.0.md)。本节及本版本子规格是 V5.11 的执行依据。
