...
For the legacy cutoff compatibility fix, code rollback is sufficient after draining retries currently being launched. Older readers ignore `researchCutoffSource` and already understand the materialized `researchCutoff`. Do not remove the materialized cutoff or replace it with rollback time. A legacy task without a valid original creation time must remain stopped and be recreated from reviewed input.
