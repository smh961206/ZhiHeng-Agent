...
Rerank only retrieved candidates with safe fallback.
...
Before V5.4.5, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- **V5.4.5-AC01:** Rerank only retrieved candidates with safe fallback. without degrading exact financial evidence retrieval.
