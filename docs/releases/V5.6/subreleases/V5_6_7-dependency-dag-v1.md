...
Connect Fact→Calculation→Claim and mark stale downstream.
...
Before V5.6.7, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- **V5.6.7-AC01:** Connect Fact→Calculation→Claim and mark stale downstream. with no silent formula semantics change.
