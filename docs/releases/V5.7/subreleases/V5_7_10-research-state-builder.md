...
Build state from structured objects, not final prose parsing.
...
Before V5.7.10, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- **V5.7.10-AC01:** Build state from structured objects, not final prose parsing. while Fact/Forecast/Assumption epistemic states remain distinct.
