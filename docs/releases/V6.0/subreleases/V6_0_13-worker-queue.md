...
Introduce idempotent distributed execution only if measured need exists.
...
Before V6.0.13, the owning release capability is either absent, partial, or still on the previous accepted implementation.
...
- **V6.0.13-AC01:** Introduce idempotent distributed execution only if measured need exists. without weakening investment-research invariants.
