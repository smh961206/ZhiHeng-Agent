...
export function parseBenchmarkArgs(args){
 const values=new Set(['--out','--suite','--kind','--repeats']),flags=new Set(['--resume']),options={};
...
export async function benchmarkCLI(args){
 ...
 const result=await runChallenger({suiteDirectory:value('--suite')??fileURLToPath(new URL('../benchmark/fixtures/bootstrap-v1',import.meta.url)),directory,
  kind:value('--kind')??'text',live:false,allowPaid:false,limits:undefined,repeats:Number(value('--repeats')??2),resume:options['--resume']===true});
