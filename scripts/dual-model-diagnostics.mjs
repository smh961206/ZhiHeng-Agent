...
try{
 ...
 phase='pro';const gateway=createModelGateway({env:{...process.env},compatibility:'legacy-text'});
 const response=await gateway.complete({purpose:'research',signal,stream:false,reasoningEffort:'off',maxOutputTokens:256,responseFormat:{type:'json_object'},messages:[{role:'user',content:'合成接口测试，仅根据以下 Vision 转写返回 JSON，字段 code（原图编号）、revenue、cost、subtraction（revenue 减 cost）。\n'+material.text}]});
