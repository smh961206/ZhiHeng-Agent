...
import {modes,knowledgeSources} from '../shared/research-framework.mjs';
import {depthSections} from '../shared/knowledge-loading.mjs';
import {assertPublishableKnowledge,createKnowledgePin} from '../shared/knowledge-engineering.mjs';
...
export function createSnapshotManager({root=projectRoot}={}){
 let active,lastError=null;
 function open(ref){
  if(!/^K\d+\.\d+\.\d+$/.test(ref?.version??'')||!/^[a-f0-9]{64}$/.test(ref?.id??''))throw new Error('Knowledge 快照标识无效；V4.x 快照不能用于当前任务');
  ...
  if(saved.id!==ref.id||saved.version!==ref.version)throw new Error('Knowledge 快照清单不一致');
  const read=path=>{
  ...
  if(catalog.schemaVersion!==3||catalog.knowledgeVersion!==ref.version||Object.hasOwn(catalog,'version')||catalog.entry!=='knowledge/ENTRY.md'||entry.match(/^\s+knowledge_version:\s*"([^"]+)"/m)?.[1]!==ref.version)throw new Error('Knowledge 入口、目录与 K-Series 版本不一致');
  assertPublishableKnowledge(catalog);
  ...
  const pin=createKnowledgePin({ref:{id:ref.id,version},catalog});
  return {ref:{id:ref.id,version},catalog,entry,manifest,read,...pin};
 }
 function current(){
  try{
   const pointer=JSON.parse(readFileSync(join(root,'knowledge/current.json'),'utf8'));
   if(pointer.schemaVersion!==1||pointer.status!=='active'||pointer.snapshotId!==pointer.fingerprint)throw new Error('Knowledge 活动指针无效');
   const next=open({id:pointer.snapshotId,version:pointer.knowledgeVersion});
   active=next;lastError=null;
  }catch(error){
   lastError=error.message;
   if(!active)throw new Error('没有可用的已激活 K-Series 快照：'+lastError);
  }
  return active;
 }
 return {current,open,status:()=>({snapshot:active?.ref??null,knowledgeVersion:active?.knowledgeVersion??null,knowledgeFingerprint:active?.knowledgeFingerprint??null,loaded:Boolean(active),verified:Boolean(active)&&!lastError,updatePending:Boolean(lastError),error:lastError})};
}
