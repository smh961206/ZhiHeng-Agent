...
export function publicModelRouting(env=process.env,state){const {analysisModel,visionModel}=modelRouting(env);return {analysisModel:state?.version===2?state.profiles.find(p=>p.id===state.active.profileId).model:analysisModel,visionModel:state?.profiles?.find(p=>p.purposes?.includes('vision'))?.model??visionModel,workflow:state?.version===2?'Vision 负责原页读取，分析模型负责研究与审计':'Vision 负责原页读取，分析模型负责研究与审计'};}
