...
export async function attachResearchBaseline(input,mode,loadJob){
 ...
 return {...input,baseline:{jobId:prior.id,question:prior.input?.question||prior.question,createdAt:prior.createdAt,
  ...
  decision:JSON.parse(JSON.stringify(prior.result.decision??null).replace(/\[(S\d+)\]/g,'[历史:$1]')),
  ...(prior.plan&&Object.hasOwn(prior.plan,'executionCompatibilityVersion')?{executionCompatibilityVersion:prior.plan.executionCompatibilityVersion,contractVersion:prior.plan.contractVersion}:{frameworkVersion:prior.plan?.version??null}),
