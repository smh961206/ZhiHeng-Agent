...
export function createSecurityIntentExtractor({env=process.env,fetchImpl=fetch,now=Date.now,timeoutMs=8000,maxConcurrent=2,maxEntries=128}={}){
 ...
 return async function extract(question,{signal}={}){
  ...
  try{
   ...
   const response=await fetchImpl(url,{method:'POST',signal:AbortSignal.any([...(signal?[signal]:[]),AbortSignal.timeout(timeoutMs)]),headers:{'Content-Type':'application/json',Authorization:`Bearer ${analysisKey}`},body:JSON.stringify({model,stream:false,max_tokens:1200,...(/^deepseek-/i.test(model)?{thinking:{type:'disabled'}}:{}),messages:[
   ...
   if(!response.ok)throw new Error('股票识别服务请求失败');
   const data=await response.json(),choice=data.choices?.[0];
   if(choice?.finish_reason!=='stop'||typeof choice.message?.content!=='string')throw new Error('股票识别输出未完成');
   result=parseSecurityIntent(choice.message.content,question);
