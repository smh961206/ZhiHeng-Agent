...
export function exportResearchMarkdown(job,{includeResearchProcess=true}={}){
 ...
 const report=deduplicateReportHeadings(job.result.report);
 ...
 const trace=(job.events??[]).filter(event=>['research_input','research_plan','tool','tool_result','fetch','fetch_error','audit_validation','audit_context','web_search','evidence_followup'].includes(event.type));
 const tools=trace.map((event,index)=>{
  const details=Object.fromEntries(['toolName','toolCallId','arguments','result','reason','issues','stopReason','contextWindow','gap','check','followup'].filter(key=>event[key]!==undefined).map(key=>[key,event[key]]));
