...
export default function App({page}){
 ...
 const [config,setConfig]=useState(null),[jobs,setJobs]=useState([]),[mode,setMode]=useState(savedDraft?.mode??'auto'),[depth,setDepth]=useState(savedDraft?.depth??'Standard'),[question,setQuestion]=useState(savedDraft?.question??''),[portfolio,setPortfolio]=useState(savedDraft?.portfolio??''),[historyYears,setHistoryYears]=useState(savedDraft?.historyYears??5),[storedJob,setSelected]=useState(null),[error,setError]=useState(''),[busy,setBusy]=useState(false),[mobileOpen,setMobileOpen]=useState(false),[contextOpen,setContextOpen]=useState(savedDraft?.contextOpen??false);
 ...
 const pathDecision=useResearchPath(question,mode,{enabled:page==='work'&&!jobId,configured:config?.configured});
 ...
 async function start(e){
  ...
  const preparation=researchPreparation({question,mode:executionMode,depth,historyYears,previousResearch,baselineJobId,portfolioContext},resolution,{pathPending:pathDecision.pending,materialsReading,materialsPending,materialEditDraft,materialDraft,config,jobs});
  ...
  try{draftStorage.flush();const payload={question,mode,depth:preparation.plan.depth,portfolio,securities:preparation.securities,historyYears:preparation.plan.historyYears,
   ...
   const options=post(payload);options.headers['Idempotency-Key']=submission.keyFor(payload);
  ...
  catch(e){if(e.status===409&&e.message.includes('路径判断已失效'))pathDecision.refresh();setError(['network','invalid_response'].includes(e.code)||e.status>=500?'暂时无法确认提交结果。输入已保留；再次提交相同输入会确认原任务，避免重复研究。':e.message);pageScroll.current?.scrollTo({top:0,behavior:'smooth'});}
