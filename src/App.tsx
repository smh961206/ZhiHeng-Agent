...
import {loadComposerDraft} from './lib/composer-draft.ts';
import {createSubmissionTracker} from './lib/research-submission.ts';
...
import {modes as frameworkModes} from './domain/research-framework.ts';

import {needsSaveRetry,deliveryProgress} from './domain/research-delivery.ts';
...
export default function App({page}){
 ...
 async function download({includeResearchProcess=true,executionOnly=false,usExchanges={}}={}){
  ...
  try{
   const {exportResearchMarkdown,exportExecutionMarkdown}=await import('./domain/research-export.ts');
 ...
 const navLink=([id,title,Icon])=><Button variant="ghost" asChild key={id} className={"nav-button "+(page===id?"nav-current":"")}><Link to={pagePaths[id]} aria-current={page===id&&!jobId?"page":undefined} onClick={()=>setMobileOpen(false)}><Icon size={18}/>{title}{id==="history"&&<span className="ml-auto text-xs opacity-70" aria-hidden="true">{jobsLoading&&!jobs.length||jobsError&&!jobs.length?'—':jobs.length}</span>}</Link></Button>;
 ...
 {page==='rules'&&<ResearchFramework onStart={newResearch} jobs={jobs} jobsLoading={jobsLoading} jobsError={jobsError}/>}
 {page==='handbook'&&<ResearchHandbookPage onStart={newResearch}/>}
 </main></div></div></div>;
}
