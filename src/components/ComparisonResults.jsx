...
export default function ComparisonResults({job,onSources}){
 ...
 const name=key=>securities.find(s=>s.market+':'+s.symbol===key)?.name||key;
 return <section className="rd-comparison" aria-label="多公司比较结果">
  <h3>比较范围与各公司判断</h3>
  <p className="rd-muted">{securities.map(s=>s.name||s.market+':'+s.symbol).join(' · ')}。质量、价格吸引力与研究优先级分别判断。</p>
  ...
  {Array.isArray(decisions)&&decisions.length?<div className="rd-comparison-grid">{decisions.map(row=><article key={row.security}>
   <h4>{name(row.security)}</h4><small>{row.security}</small><div className="rd-decision-tags"><Badge>{row.action}</Badge><Badge variant="outline">置信度 · {row.confidence}</Badge></div>
   ...
   {row.sourceIds?.length>0&&<Button variant="link" onClick={onSources}>查看依据 {row.sourceIds.join('、')}</Button>}
