...
export default function DocumentReadingSummary({job,onSources}){
 ...
 return <details className={`rd-document-reading${partial?' has-gaps':''}`}>
  ...
  <div className="rd-document-body">
   ...
   {job.modelRouting&&<>{job.modelRouting.stageModels?<dl className="rd-model-records rd-stage-models" aria-label="本次八个研究环节的模型记录">{stages.map(([key,label])=><div key={key}><dt>{label}</dt><dd>{job.modelRouting.stageModels[key]||(['criticalReviewer','judge'].includes(key)?'未启用':'未记录')}</dd></div>)}</dl>:<dl className="rd-model-records" aria-label="本次历史模型记录"><div><dt>研究配置</dt><dd>{job.modelRouting.analysisModel||'未记录'}</dd></div><div><dt>原页读取配置</dt><dd>{job.modelRouting.visionModel||'未记录'}</dd></div></dl>}<p className="rd-model-record-note">来自本次任务保存的配置；配置存在不表示已调用。{audit?'实际原页读取范围见下方记录。':'本次未保存原页复读范围，不能据此推定已经核对或没有缺口。'}</p></>}
