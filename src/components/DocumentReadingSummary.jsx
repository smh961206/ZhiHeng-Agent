...
export default function DocumentReadingSummary({job,onSources}){
 ...
 return <details className={`rd-document-reading${partial?' has-gaps':''}`}>
  ...
  <div className="rd-document-body">
   <p>视觉模型按需读取扫描页、图表和截图；分析模型结合读取内容与其他证据进行研究和审计。读取完成不代表数据已经核实。</p>
   {job.modelRouting&&<><dl className="rd-model-records" aria-label="本次模型记录"><div><dt>分析与审计</dt><dd>{job.modelRouting.analysisModel||'未记录'}</dd></div><div><dt>原页读取配置</dt><dd>{job.modelRouting.visionModel||'未记录'}</dd></div></dl><p className="rd-model-record-note">来自本次任务保存的配置；配置存在不表示已调用。实际原页读取范围见下方记录。</p></>}
