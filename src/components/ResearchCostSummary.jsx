...
export default function ResearchCostSummary({job}){
 ...
 const summaryStatus=state.status==='ready'?(!data?.complete&&!data?.calls?'记录不完整':`${data?.calls??0} 次调用`):state.status==='loading'?'读取中':state.status==='error'?'读取失败':'未读取';
 return <section className="rd-cost-summary" aria-label="服务调用与用量">
  <header className="rd-document-summary"><span className="rd-document-icon"><ReceiptText size={18}/></span><span className="rd-document-heading"><strong>服务调用与用量</strong><span>按处理环节查看次数、文本用量、缓存和费用</span></span><span className={`rd-document-label rd-cost-state is-${state.status}`}>{summaryStatus}</span></header>
  <div className="rd-document-body" aria-live="polite">
   {state.status==='idle'&&<p>需要核对实际调用次数、文本用量或费用时再读取；不会影响研究结果。</p>}
