import {reportSecurityHeadings,reportSecurities} from '../domain/report-security-headings.ts';
import {researchSecurityDisplay} from '../domain/security-display.ts';
import {useSecurityExchanges} from '../hooks/use-security-exchanges';
import {ResearchRuleUsage} from './ResearchKnowledge';
import {researchTimeline} from '../domain/research-record.ts';
import {toolResultHasGap} from '../domain/research-execution-checks.ts';
...
import {comparisonProgress} from '../domain/company-comparison.ts';
import {earningsUpdateProgress} from '../domain/earnings-update.ts';
import {deepResearchProgress} from '../domain/deep-research.ts';
...
import {researchProgress} from '../domain/research-progress.ts';
import ResearchDecision from './ResearchDecision';
import ExecutionReview from './ExecutionReview';
import {modes} from '../domain/research-framework.ts';
import {remarkReportCitations} from '../lib/report-citations.ts';
import {reportPreview} from '../domain/report-preview.ts';
...
function DetailView({job, currentConfig, tab, onTabChange, streamConnection, onRetry, retrying, retryError, onReuse, onUpdate,onDeepen, onDownload:download, exporting, onCancel, cancelling}) {
  ...
  const sources=useMemo(()=>Array.isArray(job.input?.sources)?job.input.sources:[],[job.input?.sources]);
  ...
  const outlineItems=useMemo(()=>outline.prefix===prefix+'-report'?outline.items:[],[outline.prefix,outline.items,prefix]);
  ...
  },[outlineItems,currentTab,headerHeight,toolbarHeight]);
