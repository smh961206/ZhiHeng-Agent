...
const topics=[
 ...
 {id:'progress',icon:Activity,title:'查看研究进度',summary:'创建成功后可离开页面，在研究记录中继续跟进。',items:[
  ['需要自己选择模型吗','不需要。先确认研究路径、标的与范围，平台按已启用的配置执行。模型调整先按相同问题与资料进行对照，检查事实、计算和引用；通过真实研究验收后才可启用，平台版本更新不表示本次任务已经更换模型。'],
 ...
 {id:'report',icon:ShieldCheck,title:'阅读报告与审计记录',summary:'报告、审计、来源相互对照，区分结论和待验证事项。',items:[
  ['怎样确认本次模型与规则','打开“研究过程”，在资料读取说明中查看本次保存的模型配置，在规则依据中查看实际使用的规则。配置存在不表示已调用；旧记录没有保存的配置、评估分数或调用不会按当前版本补齐。模型评估通过也不代表每份报告的结论都正确。'],
