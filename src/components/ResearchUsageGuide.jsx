...
const topics=[
 ...
 {id:'progress',icon:Activity,title:'查看研究进度',summary:'创建成功后可离开页面，在研究记录中继续跟进。',items:[
  ['需要自己选择模型吗','不需要。先确认研究路径、标的与范围，平台按各研究环节的当前配置执行。配置调整只影响之后创建的任务；已有任务沿用创建时保存的模型，不会在恢复时静默更换。'],
 ...
 {id:'report',icon:ShieldCheck,title:'阅读报告与审计记录',summary:'报告、审计、来源相互对照，区分结论和待验证事项。',items:[
  ['怎样确认本次模型与规则','打开“研究过程”，在资料读取说明中查看本次保存的模型配置，在规则依据中查看实际使用的规则。配置存在不表示已调用；旧记录没有保存的配置或调用不会按当前版本补齐。模型配置也不代表每份报告的事实与结论已经核实。'],
  ['怎样查看模型费用与缓存','打开“研究过程”中的“模型费用与缓存”，查看已记录调用、分环节费用与缓存输入用量。费用是按调用时价格记录形成的估算，不是供应商账单；价格或用量缺失时保持未知，不会按零计算。M1.0 的新任务不再设置可配置研究预算。'],
  ['关键复核和证据裁决什么时候出现','关键复核只在重复且已验证的审查问题满足条件时执行；Judge 只处理证据充分、口径明确的重大结论冲突。环节显示“已配置”仅表示可以调用，不表示本次已经执行；资料缺失或服务失败不会触发升级。请在执行轨迹核对实际调用，在审计与正文中核对保留的冲突和限制。'],
  ...
  ['怎样查看并导出完整记录','在“本次查证记录”点击“查看调用”，进入“执行轨迹”并展开“查看调用详情”。有调用编号时可按编号对照；编号显示“未记录”时，直接核对该条记录保存的输入与返回，没有保存的编号、输入或返回不会补齐。可搜索工具或筛选返回错误、未保存返回的记录；“研究过程”用于查看模型配置、规则依据、执行阶段与资料覆盖。公开计划和证据依据供复核，不包含模型私有思维链。所有研究路径默认导出完整研究记录；也可改为“报告、审计与来源”，省略工具参数和逐条事件。'],
  ...
  ['研究会一直查下去吗','为避免异常循环，研究深度设有固定安全轮次：快速筛选最多24轮、标准研究32轮、完整展开48轮。Agent可查看剩余执行额度并更新公开计划。临近上限会整理现有证据与缺口，再交给审计；网络限制、原图读取失败和未解决的证据缺口都会保留。这些安全上限不是可配置的费用预算。'],
 ...
 {id:'recovery',icon:RotateCcw,title:'处理中断与失败恢复',summary:'区分文件导入、研究执行和结果保存，按提示继续。',items:[
  ...
  ['研究失败后怎样继续','打开失败记录，先查看页面提供的恢复方式。兼容时优先沿用保存的研究或复核进度和原资料时点；未完整返回的请求可能重新发起。模型配置、规则或检查点不兼容时会说明原因，不能保证自动恢复，也不会强行混用旧计算。仅页面明确提供重新研究入口时，再确认是否从头开始。'],
