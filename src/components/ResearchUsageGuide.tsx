...
export default function ResearchUsageGuide(){
 ...
 useEffect(()=>{
  ...
  const positions=returnPositions.current;
  const id=topics.find(topic=>hash==='#usage-'+topic.id)?.id;
  if(id)reveal(id);
  else if(!hash&&scroller&&positions.has(key)){
  ...
  return()=>{
   ...
   if(!hash&&scroller)positions.set(key,{top:scroller.scrollTop,focus:section.contains(document.activeElement)?document.activeElement:null});
  };
 },[hash,key]);
