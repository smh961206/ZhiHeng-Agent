...
import {DepthScope} from './ResearchKnowledge';
...
import {executionTasks} from '../domain/execution-discipline.ts';
import {comparisonCopy} from '../domain/company-comparison.ts';
import {earningsUpdateCopy} from '../domain/earnings-update.ts';
import {deepResearchCopy} from '../domain/deep-research.ts';
...
import {researchPreparation} from '../domain/research-preparation.ts';
import {quickScreenCopy} from '../domain/quick-screen-copy.ts';
import {modes as frameworkModes,resolveMode} from '../domain/research-framework.ts';
...
export default function ResearchWorkbench({focusRequest=0,pathDecision,question,setQuestion,mode,setMode,depth,setDepth,portfolio,setPortfolio,historyYears,setHistoryYears,resolution,busy,onStart,jobs,jobsLoading,jobsError,onRefreshJobs,previousResearch,setPreviousResearch,baselineJobId,setBaselineJobId,portfolioContext,setPortfolioContext,referenceMaterials,setReferenceMaterials,materialsReading,setMaterialsReading,materialsPending,setMaterialsPending,materialDraft,setMaterialDraft,materialDraftTitle,setMaterialDraftTitle,materialEditDraft,setMaterialEditDraft,draftStatus,onClear,config}){
 ...
 return <section className="research-workbench">
  <header className="page-heading workbench-heading"><div><h1>研究工作台</h1><p>写下问题，核对公司与研究范围，再开始查证。</p></div><Button asChild variant="outline"><Link to="/handbook?tab=guide#usage-start" target="_blank" rel="noopener noreferrer" aria-label="创建研究指南（新标签页打开）">创建研究指南<ArrowUpRight size={16} aria-hidden="true"/></Link></Button></header>
    ...
    <form ref={form} onSubmit={onStart} onKeyDown={event=>{if(event.key==='Enter'&&(event.ctrlKey||event.metaKey)&&!event.nativeEvent.isComposing){event.preventDefault();if(ready&&!busy)form.current?.requestSubmit();}}}>
     <CardContent className="workbench-form-body">
      ...
      <section ref={settingsSection} tabIndex={-1} className="workbench-settings" aria-label="研究设置"><div className="field-heading"><h3>研究设置</h3><span className="settings-path-label">{settingsPending?pathDecision?.pending?"正在匹配路径":"等待选择路径":frameworkModes[effectiveMode].name}</span></div>{settingsPending?<p className="settings-awaiting">{pathDecision?.pending?"路径确认后，将显示对应设置与开始按钮。":"输入问题或选择研究路径后，这里会显示相应设置。"}</p>:<div className="options-row">
       ...
       {!settingsPending&&<Collapsible className="settings-execution-preview" aria-label={`${frameworkModes[effectiveMode].name}执行方式`}><div className="settings-execution-heading"><span><Activity size={16} aria-hidden="true"/></span><div><strong>本次研究如何展开</strong><p>{settings.processing}</p></div></div><CollapsibleTrigger asChild><Button type="button" variant="ghost" className="settings-steps-trigger">查看研究步骤<ChevronDown size={16} aria-hidden="true"/></Button></CollapsibleTrigger><CollapsibleContent><ol>{settings.steps.map((item,index)=><li key={item}><span>{String(index+1).padStart(2,'0')}</span>{item}</li>)}</ol><p className="settings-execution-note">按问题补充资料、计算与核对；缺失数据会明确列出。</p></CollapsibleContent></Collapsible>}
     ...
     </div><div className="composer-submit"><span className="composer-shortcut"><kbd>Ctrl</kbd> / <kbd>⌘</kbd> + <kbd>Enter</kbd></span><Button type="submit" size="lg" className="start-button" disabled={busy||!ready} aria-describedby="start-readiness" aria-keyshortcuts="Control+Enter Meta+Enter">{busy?<><LoaderCircle size={17} className="animate-spin"/>创建中…</>:<>{settingsPending?pathDecision?.pending?'正在匹配研究路径…':'开始研究':'开始'+settings.task}<ArrowRight size={17}/></>}</Button></div></CardFooter>
    </form>
   </Card>
