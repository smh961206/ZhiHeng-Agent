import {valuationGuidance,valuationConstraints} from './valuation-policy.ts';
// Executable routing projection of knowledge/ENTRY.md and modules/rules/.
// Source chapter bindings are checked at server startup; business rules live in those Skills.
import {researchDataCopy} from './research-data-copy.ts';
import {quickScreenCopy} from './quick-screen-copy.ts';
import {deepResearchCopy} from './deep-research.ts';
import {earningsUpdateCopy} from './earnings-update.ts';
import {comparisonCopy} from './company-comparison.ts';
import {createPublicResearchApproach} from './research-approach.ts';
import {executionIntent,executionPlan,executionBoundary} from './execution-discipline.ts';
...
export const principles=[
 {id:'quality',title:'先理解公司，再讨论价格',description:'把业务、护城河和战略，放回财务与现金流中验证。',source:'规则 4–5'},
 {id:'evidence',title:'每个判断，都能回到证据',description:'保留资料时点、来源与缺口，区分事实、解释和假设。',source:'规则 3'},
 {id:'valuation',title:'看估值区间，也看假设',description:'按行业选择方法，比较情景与敏感性，不用一个价格代替分析。',source:'规则 7'},
 {id:'falsification',title:'知道何时需要改变判断',description:'每份正式研究给出置信度与至少三条证伪条件，更新时对照旧结论。',source:'规则 9、13–14'},
];
...
export const modes={
 A:{name:'快速筛选',description:quickScreenCopy.description,question:'这家公司值得继续研究吗？',goal:quickScreenCopy.goal,example:quickScreenCopy.example,defaultDepth:'Quick',modules:['基本业务理解','五年与近期趋势','资产负债表快扫','财务红旗快扫','估值快扫','关键风险'],ruleChapters:[1,3,4,5,7,9,15,16],actions:['淘汰','观察池','深度研究']},
 B:{name:'深度研究',description:'验证长期投资逻辑',question:'长期价值由什么支撑？',goal:'连接业务、财务、现金回报和估值，形成可证伪的长期判断。',example:deepResearchCopy.example,defaultDepth:'Standard',modules:['基本面','财务质量','股东回报','估值交叉验证','决策','审计'],ruleChapters:[1,3,4,5,6,7,8,9,10,11,15,16],actions:researchActions},
 C:{name:'财报更新',description:'核对本期变化与旧判断',question:'新证据改变了哪些判断？',goal:earningsUpdateCopy.intro,example:earningsUpdateCopy.example,defaultDepth:'Standard',modules:['新旧证据对照','单季与现金流核算','关键变量与反证','参数变更','逻辑变化','审计'],ruleChapters:[1,3,5,7,9,13,14,15,16],actions:['升级','维持','降级','剔除']},
 D:{name:'多公司比较',description:'统一口径，分别形成判断',question:'哪些公司更值得深入研究？',goal:comparisonCopy.intro,example:comparisonCopy.example,defaultDepth:'Standard',modules:['比较对象与适配组','共同期间与来源核验','财务与ROE稳定性','股东回报','估值适配与逐家公司判断','审计'],ruleChapters:[1,3,4,5,6,7,9,15,16],actions:researchActions},
 E:{name:'组合分析',description:'审视配置与集中风险',question:'持仓背后的风险集中在哪里？',goal:'把组合视作一家虚拟集团，检视现金创造能力与集中风险。',example:'分析我的持仓组合，检查行业集中与现金回报',defaultDepth:'Standard',modules:['组合上下文','集中度','风险约束','审计'],ruleChapters:[1,3,9,12,15,16],actions:researchActions},
 F:{name:'股东回报',description:'关注分红与现金质量',question:'企业能持续把现金回报股东吗？',goal:'检视八年历史、三年滚动与资本配置，区分收益率锚和企业价值。',example:'研究贵州茅台近八年的分红与可持续股东回报',defaultDepth:'Standard',modules:['八年分红','三年滚动','可分配现金','收益率锚','主估值交叉验证','审计'],ruleChapters:[1,3,5,6,7,8,9,15,16],actions:researchActions},
};
...
const screenSections=[section('business','一句话商业模式'),section('financial','五年财务趋势'),section('recent','最近一期与单季变化'),section('balance','资产负债表快扫'),section('redFlags','财务红旗与反证'),section('cashFlow','现金流与资本开支'),section('valuation','估值快照与方法适用性'),section('advantages','三个优势'),section('risks','三个风险'),section('nextSteps','下一步验证重点')];
const schemas={
 ...
 Dividend:[section('snapshot','最新数据快照与口径'),section('history','八年盈利、分红与回购'),section('rolling','最新年度与三年滚动'),section('policy','分红承诺与资本配置'),section('coverage','现金覆盖、可持续性与专项评分'),section('yield','TTM股息率、可持续股息率与收益率锚'),section('sensitivity','现金回报敏感性'),section('valuation','主估值适用性及交叉验证')],
};
export function resolveMode(input={}){
 if(input.mode&&input.mode!=='auto'&&modes[input.mode])return input.mode;
 const q=input.question||'';
 if(/我的持仓|持仓分析|分析.*持仓|组合|加仓|减仓/.test(q)||Object.values(executionIntent(input)).some(Boolean))return 'E';
...
export function createResearchPlan(input={},mode=resolveMode(input)){
 ...
 ...(mode==='A'?{researchApproach:{objective:quickScreenCopy.objective+'，结论限定为淘汰、观察池或深度研究。',
  ...
  steps:[{id:'business',title:'业务是否值得理解',question:'公司如何赚钱，竞争优势有哪些可核对的证据？'},
   ...
   {id:'valuation',title:'估值快照是否适用',question:'按股类核对价格、PE、PB、币种与时点；估值方法是否匹配公司与数据口径？历史ROE均值、中位数与情景假设有哪些局限？'},
 ...
 valuationPolicy:{guidance:valuationGuidance[mode],constraints:[...valuationConstraints]},
 ...
 constraints:[...valuationConstraints,'只执行主任务与必要辅助模块','关键数据缺失须标记，降低置信度，不编造数值','任一标的缺少可读官方财报时停止研究','估值工具按行业与数据适配，收益率锚不等于内在价值','正式交付必须有研究动作、置信度与至少三条证伪条件',
 ...
 ruleBindings:{rules:[...new Set([...profile.ruleChapters,...(secondaryModules.some(m=>m.includes('股东回报'))?[6]:[]),...(secondaryModules.includes('组合约束')?[12]:[])])]}};
 ...
 plan.constraints.push(executionBoundary);
 const execution=executionPlan(input,plan.output);
 if(execution){
  plan.execution=execution;
  plan.secondaryModules.push('组合执行与交易复盘必要模块');
  plan.constraints.push(...execution.checks.map(item=>item.requirement));
  plan.requiredData.push('交易时点与当时可得证据、当前仓位和目标区间','账户规模、最小交易单位、流动性与用户执行约束','涉及立即减仓时：相关市场交易日历及未来约5个交易日的事件公告');
  plan.output.sections.push(section('execution','组合动作与执行条件'));
  if(execution.intent.retrospective||execution.intent.repurchase)plan.output.sections.push(section('transactionReview','交易复盘与重新买入核验'));
  plan.ruleBindings={rules:[...new Set([...plan.ruleBindings.rules,12,13])]};
 }
 if(mode!=='A')plan.researchApproach=createPublicResearchApproach(plan);
 return plan;
}
