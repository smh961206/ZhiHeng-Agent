import {mergeRuleRead} from '../../shared/research-knowledge.mjs';
...
export function useJobStream(selected,setSelected,setJobs){
 ...
 useEffect(()=>{
  ...
  let closed=false,finished=false,buffer='',timer,lastCheck=0,pulling=false,streamVersion=0,lastStreamAt=Date.now(),syncControl;
  ...
  const finish=job=>{if(closed||finished||!valid(job)||['queued','running'].includes(job.status))return;finished=true;clear();stream.close();update(()=>job);const {input,result,events,draft,liveReport,marketData,knowledgeUsage,...summary}=job;setJobs(list=>list.map(item=>item.id===id&&(item.retryCount??0)<=(job.retryCount??0)?{...summary,question:input.question,sourceCount:input.sources.length}:item));setConnection('');api('/api/jobs',{signal:control.signal}).then(list=>{if(!closed)setJobs(list);}).catch(()=>{});};
  async function sync(){
   if(closed||finished||pulling||Date.now()-lastCheck<5000||document.visibilityState==='hidden')return;
   pulling=true;lastCheck=Date.now();const version=streamVersion,request=new AbortController();syncControl=request;
   const deadline=setTimeout(()=>request.abort(),10000);
   try{
    const job=await api('/api/jobs/'+id,{signal:request.signal});if(closed||finished||request.signal.aborted)return;
    if(!valid(job))throw new Error('任务状态不完整');
    if(!['queued','running'].includes(job.status)){finish(job);return;}
    // Do not overwrite stream events that arrived after this read began.
    if(version===streamVersion){clear();update(()=>job);}setConnection('');
   }catch{if(!closed&&!finished)setConnection('暂时无法同步进度，将自动重试；刷新页面不会取消研究。');}
   finally{clearTimeout(deadline);if(syncControl===request)syncControl=undefined;pulling=false;}
  }
  ...
  listen('snapshot',job=>{if(!valid(job))throw new Error('Invalid snapshot');if(!['queued','running'].includes(job.status))finish(job);else{clear();update(()=>job);}});
  listen('workflow',workflow=>{if(!Array.isArray(workflow.stages))throw new Error('Invalid workflow');update(job=>({...job,workflow}));});
  listen('trace',entry=>update(job=>({...mergeRuleRead(job,entry),events:[...job.events,entry]})));
  ...
  return()=>{closed=true;control.abort();syncControl?.abort();clearInterval(poll);window.removeEventListener('focus',focus);document.removeEventListener('visibilitychange',focus);clear();stream.close();};
