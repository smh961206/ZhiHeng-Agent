...
export function registerAgentCapabilityScenarios({test,makeJob,detail,detailActions,downloadReport,textIncludes,count,noOverflow,screenshot}){
 for(const width of [320,1440]){
  ...
  test('deep-return-receipts-'+width,{jobs:[job],viewport:{width,height:1100}},async({page})=>{
   ...
   await textIncludes(panel,'基准调用缺失');const receiptSelect=panel.getByRole('combobox',{name:'选择计算记录',exact:true});assert.equal(await panel.locator('select').count(),0);assert.equal(await panel.locator('label[for="'+await receiptSelect.getAttribute('id')+'"]').getAttribute('data-slot'),'label');assert.equal(await receiptSelect.getAttribute('data-slot'),'select-trigger');
   await receiptSelect.click();await count(page.getByRole('option'),3);await page.getByRole('option',{name:'2. 复算分红事件与支付率 · ledger-record',exact:true}).click();
   ...
   await receiptSelect.click();assert.equal(await page.getByRole('option',{name:'2. 复算分红事件与支付率 · ledger-record',exact:true}).getAttribute('aria-selected'),'true');await screenshot(page,'deep-return-select-open-'+width,'.research-analysis-receipts');await page.getByRole('option',{name:'1. 核算增长与折现率网格 · grid-record',exact:true}).click();await textIncludes(panel,'不可计算');await textIncludes(panel,'120.00');await textIncludes(panel,'base-cash');
