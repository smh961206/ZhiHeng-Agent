...
test('creation API deduplicates requests, preserves UTF-8 chunks, and never restores deleted submissions',{timeout:20000},async t=>{
 ...
 const requestFetch=(url,options={})=>fetch(url,{...options,signal:t.signal});
 ...
 try{
  ...
  let ready=false;for(let n=0;n<100;n++){t.signal.throwIfAborted();try{if((await requestFetch(base+'/api/health')).ok){ready=true;break;}}catch{}await pause(30);}assert.ok(ready,output);
  phase='chunked UTF-8 creation';
  const bytes=Buffer.from(JSON.stringify(input)),cut=bytes.indexOf(Buffer.from('中文'))+1;
  const first=await new Promise((resolve,reject)=>{
   const request=http.request(base+'/api/jobs',{method:'POST',headers:post(input).headers,signal:t.signal},response=>{let text='';response.on('data',chunk=>{text+=chunk;});response.on('end',()=>resolve({status:response.statusCode,job:JSON.parse(text)}));});
  ...
  phase='deduplication';
  const repeated=await Promise.all([requestFetch(base+'/api/jobs',post(input)),requestFetch(base+'/api/jobs',post(input))]);
  ...
  assert.equal((await requestFetch(base+'/api/jobs',post({...input,question:'改变输入'}))).status,409);
  phase='cancellation persistence';
  const path='/api/jobs/'+first.job.id;await requestFetch(base+path+'/cancel',{method:'POST'});
  for(let n=0;n<100;n++){t.signal.throwIfAborted();if((await storage.getJob(first.job.id)).status==='cancelled')break;await pause(20);}
  ...
  phase='deleted submission protection';
  const replay=await requestFetch(base+'/api/jobs',post(input));assert.equal((await replay.json()).status,'cancelled');
  assert.equal((await requestFetch(base+path,{method:'DELETE'})).status,200);
  assert.equal((await requestFetch(base+'/api/jobs',post(input))).status,409);assert.equal((await storage.listJobs()).length,0);
  phase='completed';
 }finally{
