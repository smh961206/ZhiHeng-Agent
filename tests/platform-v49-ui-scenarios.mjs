...
export async function registerPlatformV49Scenarios({test,makeJob,detail,openProcess,textIncludes,noOverflow,screenshot}){
 ...
 test('platform-v49-deferred-home-modules',{},async({page,requests})=>{
  ...
  const trigger=page.getByRole('button',{name:'平台运行说明',exact:true}),requested=page.waitForRequest(statusModule);await trigger.click();
 ...
 for(const width of [320,1440,2560]){
  test('platform-v49-pages-'+width,{viewport:{width,height:1000}},async({page,requests})=>{
   ...
   await textIncludes(panel,'按问题研究');await textIncludes(panel,'过程可回查');
   await textIncludes(panel,'具体资料能否取得，以本次研究结果为准');
  ...
  test('platform-v49-guide-navigation-'+width,{viewport:{width,height:1000}},async({page,requests})=>{
   await page.goto('/handbook?tab=guide#usage-materials');
   ...
   assert.equal(await page.getByRole('navigation',{name:'操作教程快速定位',exact:true}).count(),0);
   ...
   await page.reload();await page.waitForFunction(()=>document.activeElement?.id==='usage-materials');
  ...
  test('platform-v49-guide-deep-link-history-'+width,{viewport:{width,height:1000}},async({page,requests})=>{
   await page.goto('/handbook');
   await page.goto('/handbook?tab=guide#usage-materials');
   await page.waitForFunction(()=>document.activeElement?.id==='usage-materials');
   await page.goBack();assert.equal(new URL(page.url()).hash,'');
   await page.goForward();await page.waitForFunction(()=>document.activeElement?.id==='usage-materials');
   await textIncludes(page.locator('#usage-materials'),'截图和扫描件怎样准备');
   await noOverflow(page,'guide deep link history '+width);
   assert.equal(requests('POST','/api/jobs').length,0);
  });
 }
