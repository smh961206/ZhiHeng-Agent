...
export function registerPlatformV51Scenarios({test,makeJob,detail,openProcess,textIncludes,noOverflow,screenshot}){
 for(const width of [320,1440])for(const variant of ['known','unknown','error']){
  ...
  test(`platform-v51-cost-${variant}-${width}`,{jobs:[job],viewport:{width,height:1100}},async({page})=>{
   ...
   await detail(page,job);await openProcess(page);const panel=page.locator('.rd-cost-summary');await panel.locator('summary').click();
   ...
   assert.match(await panel.innerText(),/模型费用与缓存/);assert.doesNotMatch(await panel.innerText(),/研究预算|预算限制|API_KEY|prefixFingerprint|reasoning_content/);await noOverflow(page,'cost '+width);await screenshot(page,`v51-cost-${variant}-${width}`,'.rd-cost-summary');
   await panel.getByRole('button',{name:'刷新费用记录'}).click();await textIncludes(panel,variant==='error'?'暂时无法加载':'费用');
  });
 }
}
