...
export function registerWorkflowScenarios({test,workbench,manualInput,enabled,textIncludes,count,collapsible,noOverflow,screenshot}){
 for(const width of [320,1440])test(`preparation-scope-and-validation-${width}`,{viewport:{width,height:1000}},async({page,requests})=>{
  ...
  assert.equal(await preparation.locator('.preparation-outline').getAttribute('open'),null);
  await textIncludes(preparation.locator('.input-optional'),'选填');
  await screenshot(page,'workbench-scope-card-'+width);
  await preparation.locator('.preparation-outline>summary').click();await textIncludes(preparation,'验证计划 · 证据判断 · 估值区间与研究动作');await textIncludes(preparation,'审计记录与证据来源');await count(preparation.locator('.preparation-sections'),0);await preparation.locator('.preparation-outline>summary').click();
  ...
  await preparation.locator('.preparation-outline>summary').focus();await page.keyboard.press('Enter');
  await textIncludes(preparation.locator('.preparation-outline'),'分红可持续性 · 收益率锚 · 交叉验证');
  await count(preparation.locator('.research-scene-deliverables>li'),8);
  await noOverflow(page,'delivery preview '+width);await screenshot(page,'preparation-delivery-'+width,'.preparation-delivery');
  await preparation.locator('.preparation-outline>summary').click();
